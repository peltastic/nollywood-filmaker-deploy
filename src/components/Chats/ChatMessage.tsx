...
import { FaDownload, FaExternalLinkAlt, FaReply } from "react-icons/fa";
import Link from "next/link";
import { useSelector } from "react-redux";
import { RootState } from "@/lib/store";
import { AspectRatio, RingProgress } from "@mantine/core";
...
const ChatMessage = React.memo(
  ({
    ...
    const springX = useSpring(x, { stiffness: 400, damping: 40 }); // Spring for smooth return
    ...
    // Handle drag end to trigger reply and reset position
    const handleDragEnd = (event: any, info: any) => {
      const dragDistance = info.offset.x;
      if (dragDistance > 10 && id && setReplyDataProps) {
      ...
      // Explicitly reset position to 0
      x.set(0);
    };
        ...
        >
          ...
          >
            ...
            <div className="relative">
              ...
              <motion.div
                className="absolute inset-0 flex items-center justify-start pl-4 bg-gray-200 text-gray-600 rounded-xl pointer-events-none"
                style={{ opacity: x.get() > 0 ? Math.min(x.get() / 80, 1) : 0 }}
              >
              ...
              <motion.div
                ref={messageRef}
                style={{ x: springX }} // Use spring animation for position
                ...
                } py-2 sm:py-3 px-2 sm:px-3 text-[0.8rem] sm:text-[0.95rem] rounded-xl max-w-[12rem] sm:max-w-[20rem] relative select-text`}
              >
